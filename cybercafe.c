...
char password[20];
 ...
 scanf("%s",&password);
 if(strcmp(username, "Prashanth")==0)
 {
 if(strcmp(password ,"Prashanth123")==0)
 {
 system("CLS");
 main_heading();
...
char choice='y';
 ...
 fprintf(fp,"%02d/%02d/%d %02d:%02d",current_time->tm_mday,current_time-
>tm_mon+1,current_time->tm_year+1900,current_time->tm_hour,current_time->tm_min);
 fprintf(fp,"\n");
 arrival_time();
...
return;
 ...
 //printf("\n\t\t\t ____");
 while(fscanf(fp,"%d %s %02d/%02d/%d
%02d:%02d",&id,&name,&mday,&mmon,&myear,&mhour,&mmin)!=EOF)
 {
 printf("\n\n\t\t\t %d \t\t\t%s \t\t\t%02d/%02d/%d
\t%02d:%02d",id,name,mday,mmon,myear,mhour,mmin);
...
%02d:%02d",&id,&name,&mday,&mmon,&myear,&mhour,&mmin)!=EOF)
 ...
 printf("\n\n\t\t\t\t\tUser_ID : %d",id);
 printf("\n\t\t\t\t\tName of User : %s",name);
 printf("\n\t\t\t\t\tLogin Date And Time : %02d/%02d/%d
%02d:%02d",mday,mmon,myear,mhour,mmin);
tothr =(((current_time->tm_hour*60)+(current_time->tm_min))-
((mhour*60)+mmin));
 duration = tothr/60;
 totmin = tothr-(duration*60);
 ...
 if(my_id!=id)
{
 ...
 fclose(fp);
 fclose(ft);
 remove("cybercafe.txt");
 rename("cybercafe1.txt","cybercafe.txt");
